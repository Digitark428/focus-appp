...
export default function FocusApp() {
  ...
  // ============================================================
  // === END-TASK POPUP VARIABLES (moved out of IIFE for esbuild compat) ===
  // ============================================================
  const etpIsInProgress = endTaskPopup ? currentTask?.id === endTaskPopup.id : false;
  let etpMinutesSaved = 0;
  if (endTaskPopup && etpIsInProgress) {
    const etpTaskEnd = toMin(endTaskPopup.end);
    let etpNowMinutes;
    if (demoMode) {
      const etpElapsed = demoElapsed % DEMO_TASK_DURATION;
      const etpRatio = etpElapsed / DEMO_TASK_DURATION;
      const etpDur = toMin(endTaskPopup.end) - toMin(endTaskPopup.start);
      etpNowMinutes = toMin(endTaskPopup.start) + etpRatio * etpDur;
    } else {
      etpNowMinutes = now.getHours() * 60 + now.getMinutes() + now.getSeconds() / 60;
    }
    etpMinutesSaved = Math.max(0, Math.round(etpTaskEnd - etpNowMinutes));
  }
              ...
              {!showExtendChoice ? (
                <>
                  <div className="flex flex-col items-center text-center mb-6">
                    ...
                    <p className="text-xs uppercase tracking-[0.2em] text-white/40 mb-2">
                      {etpIsInProgress ? "Tâche en cours" : "Tâche terminée"}
                    </p>
                  ...
                  <div className="space-y-2">
                    {etpIsInProgress && etpMinutesSaved > 0 && (
                      <button onClick={() => finishEarly(endTaskPopup)}
                        ...
                        <span className="text-sm font-medium">
                          J'ai déjà fini · gagner {etpMinutesSaved} min
                        </span>
                      </button>
                    )}
                    {(!etpIsInProgress || etpMinutesSaved === 0) && (
                    ...
                    <button onClick={() => setShowExtendChoice(true)}
                      ...
                      <Plus size={14} />
                      {etpIsInProgress ? "Pas encore, ajouter du temps" : "Non, ajouter du temps"}
                    </button>
                    <button onClick={() => {
                        if (etpIsInProgress) {
                          popupOpenedAtRef.current = null;
                      ...
                      className="w-full py-2.5 text-xs text-white/40 hover:text-white/70 transition">
                      {etpIsInProgress ? "Continuer normalement" : "Passer à la prochaine tâche"}
                    </button>
                  </div>
                </>
              ) : (
